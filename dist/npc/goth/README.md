...
Starting a game or opening chat reuses an ongoing background load. Closing the conversation clears its messages while an automatic model load can continue. Disabling the setting prevents automatic loading on subsequent page loads; it does not delete cached model files or interrupt a load that has already started.
