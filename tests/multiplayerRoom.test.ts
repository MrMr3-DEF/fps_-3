...
test('BinaryPack room: visibility, every weapon in both directions, respawns and C health', { timeout: 15000 }, async () => {
    ...
    try {
        ...
        await call(0, 'pose', { z: 10, yaw: -0.055 });
        await call(1, 'pose', { z: -10, yaw: Math.PI - 0.055 });
        await flush();
        ...
        await call(0, 'scope', { enabled: true });
        for (let i = 0; i < 10; i++) await tick();
        await call(0, 'shoot', { weapon: 'PISTOL' }); await flush();
        for (let i = 0; i < 3; i++) await tick();
        assert.equal((await call(1, 'state')).hp, 9, 'host homing pistol corrects a scan-box near miss');
        assert.equal((await call(0, 'state')).peers[guest.peerId].hp, 9, 'host inspection agrees with guest health');
        await call(1, 'scope', { enabled: true });
        for (let i = 0; i < 10; i++) await tick();
        await call(1, 'shoot', { weapon: 'PISTOL' }); await flush();
        for (let i = 0; i < 3; i++) await tick();
        assert.equal((await call(0, 'state')).hp, 9, 'guest homing hit passes host validation');
